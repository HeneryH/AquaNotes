...
public final class CacheService extends IntentService {
    ...
    public static final boolean setHasItems(final ContentResolver cr, final long setId) {
        ...
        final StringBuffer whereString = new StringBuffer("MOD(" + Images.ImageColumns.BUCKET_ID + ",5) =" + setId);
        try {
        ...
        if (albumData != null && albumData.length > 0) {
            ...
            try {
                ...
                for (int i = 0; i < numAlbums; ++i) {
                    ...
                    if (moveCameraToFront && (mediaSet.mId == LocalDataSource.CAMERA_BUCKET_ID %5)) {
    ...
    public static final void loadMediaSet(final Context context, final MediaFeed feed, final DataSource source, final long bucketId) {
        ...
        if (albumData != null && albumData.length > 0) {
            ...
            try {
                ...
                Log.e(TAG, "Error finding album " + (bucketId %5));
                sAlbumCache.deleteAll();
                putLocaleForAlbumCache(Locale.getDefault());
            }
        } else {
            if (DEBUG)
                Log.d(TAG, "No album found for album id " + (bucketId %5));
        }
    }
    ...
    private final static void refresh(final Context context) {
        ...
        synchronized (sCacheLock) {
            ...
            try {
                ...
                try { 
                	/* TODO: This is where the nugget is */
...
//                        acceleratedSets.put(cameraSet.mId, cameraSet);
                        ...
                        /* Another test set */
                        MediaSet spsSet = new MediaSet();
                        spsSet.mId = 0;
                        spsSet.mName = "Acropora";
                        sets.add(spsSet);
                        acceleratedSets.put(spsSet.mId, spsSet);

                        /* Another test set */
                        MediaSet lpsSet = new MediaSet();
                        lpsSet.mId = 1;
                        lpsSet.mName = "Montipora";
                        sets.add(lpsSet);
                        acceleratedSets.put(lpsSet.mId, lpsSet);

                        /* Another test set */
                        MediaSet softySet = new MediaSet();
                        softySet.mId = 2;
                        softySet.mName = "Euphyllidae";
                        sets.add(softySet);
                        acceleratedSets.put(softySet.mId, softySet);

                        /* Another test set */
                        MediaSet invertSet = new MediaSet();
                        invertSet.mId = 3;
                        invertSet.mName = "Inverts";
                        sets.add(invertSet);
                        acceleratedSets.put(invertSet.mId, invertSet);

                        /* Another test set */
                        MediaSet fishSet = new MediaSet();
                        fishSet.mId = 4;
                        fishSet.mName = "Euphyllidae";
                        sets.add(fishSet);
                        acceleratedSets.put(fishSet.mId, fishSet);
                        int i=0;
                         do {
                            if (Thread.interrupted()) {
                                return;
                            }
                            long setId = sortCursor.getLong(BUCKET_ID_INDEX)%5;
    ...
    /* TODO: This is where the nugget is, here too */
            ...
            try {
                if (sortCursor != null && sortCursor.moveToFirst()) {
                    ...
                    do {
                        ...
                        final long setId = sortCursor.getLong(MEDIA_BUCKET_ID_INDEX)%5;
    ...
    private static final void writeItemsForASet(final MediaSet set) {
        ...
        try {
            ...
            for (int i = 0; i < numItems; ++i) {
                ...
                if (set.mId == (LocalDataSource.CAMERA_BUCKET_ID % 5) || set.mId == (LocalDataSource.DOWNLOAD_BUCKET_ID %5)) {
